function nucleiFilteredPositions = ...
    BOProfileDescriptor2D(imageIn,nucleiPositions,descriptorTable,...
...
while(stop>0)
    ...
    for i=1:tableSize
        if i~=j 
            d = sqrt( (nucleiPositions_tmp(i,1) ... 
                - nucleiPositions_tmp(j,1))^2 ...
                + (nucleiPositions_tmp(i,2) ...
                - nucleiPositions_tmp(j,2))^2 );
            if m > d
                m = d; ix = i; jx = j;
            end
        end
    ...
    if (m<distancemaxXY)
        ...
        lineMask = func_DrawLine(lineMask,nucleiPositions_tmp(ix,1), ...
            nucleiPositions_tmp(ix,2), nucleiPositions_tmp(jx,1), ...
            nucleiPositions_tmp(jx,2), 1);
